...
async function parseFunctionError(res){if(!res)return 'Falha na comunicação com o servidor.';if(res.data?.error)return res.data.error;if(res.error){if(typeof res.error.context?.json==='function'){try{const b=await res.error.context.json();if(b?.error)return b.error}catch{}}return res.error.message||String(res.error)}return null}
...
function openUser(user=null){
 $('#userForm').reset();$('#userId').value=user?.id||'';$('#userDialogTitle').textContent=user?'Gerenciar acesso':'Cadastrar usuário';$('#userDialogSubtitle').textContent=user?`${user.email} · deixe a nova senha vazia para mantê-la`:'Crie o acesso e defina a responsabilidade da pessoa.';$('#userName').value=user?.nome||'';$('#userEmail').value=user?.email||'';$('#userEmail').disabled=!!user;$('#userLogin').value=user?.login||'';$('#userRole').value=user?.role||'supervisor';$('#userActive').checked=user?!!user.ativo:true;$('#userPasswordLabel').textContent=user?'Nova senha (opcional)':'Senha inicial';$('#userPassword').required=!user;$('#userPasswordConfirm').required=!user;$('#activeField').style.display=user?'flex':'none';$('#userSubmit').textContent=user?'Salvar alterações':'Cadastrar usuário';const self=user?.id===profile.id;$('#userRole').disabled=self;$('#userActive').disabled=self;showDialog('userDialog','#userName')
}
...
async function toggleUser(user){if(user.id===profile.id)throw new Error('Seu próprio acesso de administrador é protegido.');const next=!user.ativo;if(!confirm(`${next?'Ativar':'Desativar'} o acesso de ${user.nome}?`))return;const res=await supabase.functions.invoke('admin-update-user',{body:{user_id:user.id,nome:user.nome,login:user.login,role:user.role,ativo:next}});const err=await parseFunctionError(res);if(err)throw new Error(err);toast(`Acesso de ${user.nome} ${next?'ativado':'desativado'}.`);await reload('usuarios')}
async function resendUser(user){const redirect_to=`${location.origin}${location.pathname.replace(/admin\.html.*$/,'reset-password.html')}`;const res=await supabase.functions.invoke('admin-reset-password',{body:{user_id:user.id,redirect_to}});const err=await parseFunctionError(res);if(err)throw new Error(err);toast(`E-mail de acesso enviado para ${res.data?.email||user.email}.`)}
...
$('#userForm').onsubmit=async event=>{
 ...
 try{const password=$('#userPassword').value,passwordConfirm=$('#userPasswordConfirm').value;if(password!==passwordConfirm)throw new Error('As senhas não coincidem.');if(!id&&password.length<8)throw new Error('A senha precisa ter pelo menos 8 caracteres.');const payload={nome:$('#userName').value.trim(),login:$('#userLogin').value.trim().toLowerCase(),role:$('#userRole').value,ativo:id===profile.id?true:$('#userActive').checked};let response;if(id)response=await supabase.functions.invoke('admin-update-user',{body:{...payload,user_id:id,...(password?{password}:{})}});else response=await supabase.functions.invoke('admin-create-user',{body:{...payload,email:$('#userEmail').value.trim().toLowerCase(),password}});const err=await parseFunctionError(response);if(err)throw new Error(err);closeDialog('userDialog');toast(id?(password?'Acesso e senha atualizados.':'Acesso atualizado e registrado.'):'Usuário cadastrado com sucesso.');await reload('usuarios')}catch(error){message.textContent=error.message||String(error)}finally{setBusy(button,false)}
};
