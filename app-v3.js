...
$('#editUserForm').onsubmit=async e=>{
 ...
 const res=await supabase.functions.invoke('admin-update-user',{body:payload});const err=await parseFunctionError(res);
 showMsg('#editUserMsg',err?new Error(err):null,'Alterações salvas e registradas.');
 if(!err){if(editingUserId===profile.id){profile={...profile,...res.data?.user};$('#who').textContent=profile.nome||profile.email;$('#avatar').textContent=initials(profile.nome)}await loadGovernance();setTimeout(closeUserEditor,650)}
 button.disabled=false;button.textContent='Salvar alterações'
};
$('#userForm').onsubmit=async e=>{e.preventDefault();const button=e.submitter||e.target.querySelector('button[type="submit"]');button.disabled=true;button.textContent='Cadastrando...';const payload={nome:$('#uNome').value.trim(),email:$('#uEmail').value.trim(),login:$('#uLogin').value.trim().toLowerCase(),password:$('#uSenha').value,role:$('#uRole').value};const res=await supabase.functions.invoke('admin-create-user',{body:payload});const err=await parseFunctionError(res);showMsg('#userMsg',err?new Error(err):null,'Usuário cadastrado e registrado na auditoria.');if(!err){e.target.reset();await loadGovernance();await loadStructure()}button.disabled=false;button.textContent='Cadastrar usuário'};
