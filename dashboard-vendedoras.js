...
async function saveSeller(doc){
 ...
 const inputs=[...screen.querySelectorAll('input')];if(inputs.length<7)throw new Error('Abra o formulário Nova vendedora e preencha os dados.');
 const [nomeI,lojaI,metaI,vendaI,ticketI,cpfI,admI]=inputs.slice(-7),nome=nomeI?.value?.trim(),lojaNome=lojaI?.value?.trim();if(!nome||!lojaNome)throw new Error('Informe nome e loja.');
 ...
 const cpf=cpfI?.value?.trim()||null,admissao=admI?.value?.trim()||null,meta=parse(metaI?.value),venda=parse(vendaI?.value),ticket=parse(ticketI?.value);
